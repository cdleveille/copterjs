...
-   Create and populate a `.env` file in the root folder based on the `.env.example` file, which contains default environment variable values for a development environment.
    -   Optionally set the `USE_DB` environment variable to `false` and skip the "Set up database" instructions below if there is no need for a dev database.

Set up dev database (only needed if `USE_DB=true` in the `.env` file):
